...
def fit_model_and_print_metrics(skf_splits,X_not_val, y_not_val, classifier, print_metrics = False):
    ...
    for train_ind, test_ind in skf_splits:
        ...
        accuracy_list.append(accuracy_score(y_test, y_hat))
        precision_list.append(precision_score(y_test, y_hat, average='weighted'))
        recall_list.append(recall_score(y_test, y_hat, average='weighted'))
        f1_list.append(f1_score(y_test, y_hat, average='weighted'))
    
        #roc_auc_list.append(roc_auc_score(y_test, y_hat, average=None))
